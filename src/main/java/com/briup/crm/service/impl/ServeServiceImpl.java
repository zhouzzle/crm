...
public class ServeServiceImpl implements ServeService {
	...
	@Override
	public PageInfo<CstService> findServiceLikeByTypeAndStatus(CstService serve, int curPage, int size) {
		PageHelper.startPage(curPage,size);
		CstServiceExample example = new CstServiceExample();
		example.createCriteria().andSvrStatusLike("%"+serve.getSvrStatus()+"%")
		.andSvrTypeLike("%"+serve.getSvrType()+"%");
		List<CstService> servicelist = serviceMapper.selectByExample(example);
		PageInfo<CstService> serviceinfo = new PageInfo<CstService>(servicelist);
		return serviceinfo;
	}
