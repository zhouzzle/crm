	...
	$(function(){
		...
		$('.editUser').click(function(){
			...
			$.get(url,function(data){
				...
				$("select[name=region]").val(data.chcAddr);
				$("input[name=linkman]").val(data.chcLinkman);
				$("input[name=tel]").val(data.chcTel);
				$("input[name=desc]").val(data.chcDesc);
				$("input[name=createBy]").val(data.chcCreateBy);
				$("select[name=dueTo]").val(data.chcDueTo);
			});
			$('#toAdd').show();
		})
		// 查询
