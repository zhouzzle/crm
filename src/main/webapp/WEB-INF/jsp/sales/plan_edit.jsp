		 ...
		 <thead>
		  ...
		  <tbody>
		    <c:forEach items="${chance.plans}" var ="plan">
		       ...
		       <td>
		      	<i  value="${plan.plaId }" class="layui-icon layui-icon-edit editUser"></i>  
		      	<i  value="${plan.plaId }" class="layui-icon layui-icon-delete deleteUser"></i>   
		      </td>
		    </tr> 
		  </c:forEach> 
         </tbody>
...
<script type="text/javascript">
	$(function(){
		...
		$('.btn-primary').click(function(){
			var url = "plan/deletePlanByPlanId/"+planId;
			$('.layui-body').load(url);  
			$('#deleteModal').hide();
		})
		// 点击新增显示模态框
		...
		$('.editUser').click(function(){
			planId = $(this).attr("value");
			var url = "plan/findPlanByPlanId/"+planId;
			console.log()
			$.get(url,function(data){
				$("input[name=plan]").val(data.plaTodo);
